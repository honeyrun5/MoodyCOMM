...
class ViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func loginPressed() {
        ...
        let urlString: String = "http://condi.swu.ac.kr/student/T08/login/loginUser.php"
        ...
        //로그인 성공 시에 사용자 아이다와 이름을 appDelegate에 저장한다.
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        appDelegate.ID = self.loginUserid.text
        appDelegate.userName = name
            
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
            let naviViewController = storyboard.instantiateViewController(withIdentifier: "tabView")
